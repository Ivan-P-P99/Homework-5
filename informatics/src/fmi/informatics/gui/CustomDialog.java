...
public class CustomDialog extends JDialog implements ActionListener, PropertyChangeListener {
	...
	// Създаване на диалог
	public  CustomDialog(String text, PersonDataGUI parent,SortOrder sortOrder) {
		...
		if (isVisible() && (evt.getSource() == optionPane)
			...
			if (value.equals(okLabel)) {
				if (textField.getText() != null && !textField.getText().isEmpty()) {
					...
					if (CheckIfNum(typedText)) {
						intValue = Integer.parseInt(typedText);
						
						if (intValue >= 1 && intValue <= 5) {
							parentGui.sortTable(intValue, parentGui.table, PersonDataGUI.people, null);
							clearAndHide();
						} else {
							// Текстът е невалиден
							textField.selectAll();
						...
						JOptionPane.showMessageDialog(CustomDialog.this,
								"Съжаляваме, трябва да въведете число!", 
								"Грешка", JOptionPane.ERROR_MESSAGE);
					}
	...
	private static boolean CheckIfNum(String typedText) {
		try {
